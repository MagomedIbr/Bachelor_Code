...
def filteremg( emg, low_pass=10, sfreq=600, high_band=10, low_band=200):
    """
    time: Time data
    emg: EMG data
    ...
    for x in range(5):
        #emg = emg[:,x]
        emg_filtered = sp.signal.filtfilt(b1, a1, emg[:,x])
    return emg		
...
# print (df)
df.to_csv(temp_folder+'/'+'allEMGData10too200hz_with_audible_only_sessions.csv',index=False)
